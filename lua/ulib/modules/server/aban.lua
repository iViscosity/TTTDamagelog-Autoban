...
function ABAN.BanPlayer(ply, slays)
	...
	local secs = time * 60
	timeStr = ULib.secondsToStringTime(secs)
...
-- The main function that checks if someone should be banned
local function CheckForSlays(ply)
	...
	if ULib.bans[ply:SteamID()] then return end -- don't need to do anything if they were already banned (triggering this hook)
	if exempt:GetBool() and ply:CheckGroup(lowest:GetString()) then return end -- check if certain ranks are exempt from being banned, and if this person inherits from that group
	...
	if not slays or slays == 0 or slays < minslays:GetInt() then return end
